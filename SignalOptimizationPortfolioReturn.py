...
def TradingOnSignal(luvdf, priceReturn):
    ...
    signals['ind_short_mavg'] = np.array(priceReturn.mean(axis=1).rolling(window = shortWindow, min_periods = 1, center = False).mean())
    signals['ind_long_mavg'] = np.array(priceReturn.mean(axis=1).rolling(window = longWindow, min_periods = 1, center = False).mean())
    ...
    # Create signals
    signals['ind_signal'][shortWindow:] = np.where(signals['ind_short_mavg'][shortWindow:] > signals['ind_long_mavg'][shortWindow:], 1.0, 0.0)   
...
def main():
    ...
    signalPoints = TradingOnSignal( luvdf, priceReturn)

    #print the total trading points 
    print(sum("Total Signal Points : " ,signalPoints["luv_signal"]))
    ...
    ax1.plot(signalPoints.loc[signalPoints.positions == 1.0].index, 
            signalPoints.luv_short_mavg[signalPoints.positions == 1.0],
    ...
    ax1.plot(portfolio.loc[signalPoints.positions == 1.0].index, 
            portfolio.total[signalPoints.positions == 1.0],
            '^', markersize = 10, color='m')
    ax1.plot(portfolio.loc[signalPoints.positions == -1.0].index, 
            portfolio.total[signalPoints.positions == -1.0],
            'v', markersize = 10, color='k')
